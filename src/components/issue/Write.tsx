...
const Write = ({
  ...
  return (
    <WriteStyle>
      ...
      <DetailTagStyle>
        {tag &&
          tag.map((value: any, index: any) => {
            return (
              <span onClick={() => setDeleteTag(index)} key={index}>
                # {value}
              </span>
            );
          })}
      ...
      <IssueEditor editorRef={editorRef} height="300px" />
      {/* <TipTap isEditable={true} height="300px" mode="editor" editorRef={editorRef} /> */}
      <Button fullWidth onClick={onIssueWrite}>
        이슈 등록
      </Button>
      <Button fullWidth onClick={onIssueWriteTemp}>
        임시 저장
      </Button>
      <br />
    </WriteStyle>
